...
class GraphAttentionLayer(nn.Module):
    ...
    """
    def __init__(self, configs, att_head, in_dim, out_dim, dp_gnn, leaky_alpha=0.2):
    ...
    def forward(self, feat_in, adj=None):
        batch, N, in_dim = feat_in.size()  # (bs, curr_max_win_len, feat_dim) / (bs, curr_max_win_len, feat_dim + emo_dim)
        ...
        attn.data.masked_fill_(mask.byte(), -999)  # 填充部分（mask=1） attention分数置为很小的负数

        attn = F.softmax(attn, dim=-1) # 填充部分 attention分数 取exp 计算权重 就接近0
